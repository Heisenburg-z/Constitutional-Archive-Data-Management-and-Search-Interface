...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    { title: 'Storage Used', value: formatFileSize(recentUploads.reduce((acc, doc) => acc + (doc.fileSize || 0), 0))+" MB", icon: Folder },
  ...
  const generateReportContent = (documents, users, userProfile) => {
    ...
    const reportSections = [
      ...
      ...users.map(user => {
        ...
        const userDocs = files.filter(file => {
          // Compare string representations of the IDs
          return file.createdBy?.toString() === user._id?.toString();
        }).length;
        console.log('User:', user, 'User Docs:', userDocs);
