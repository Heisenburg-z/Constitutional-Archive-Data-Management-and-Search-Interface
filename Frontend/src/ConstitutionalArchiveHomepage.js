...
import { Search, ArrowRight, BookMarked, X, Loader2, AlertTriangle, ExternalLink, MessageSquare, ThumbsUp, ThumbsDown } from 'lucide-react';
    ...
    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 relative">
      ...
      {feedback.submitted ? (
        ...
        <>
          ...
          <p className="text-gray-600 mb-6">How would you rate your experience with Constitutional Archive?</p>
            ...
            <div className="flex justify-center space-x-2 mb-6">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`text-3xl ${feedback.rating >= star ? 'text-yellow-400' : 'text-gray-300'} hover:text-yellow-400 transition-colors`}
      ...
      <section className={`py-20 px-6 text-center max-w-4xl mx-auto ${searchResults ? 'pb-8' : ''}`}>
        ...
        <form onSubmit={handleSearchSubmit} className="relative max-w-2xl mx-auto">
          ...
          <input 
            ...
            className="w-full p-4 pl-12 pr-24 text-lg rounded-xl border-2 border-blue-200 shadow-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none transition"
          />
          <Search className="absolute left-4 top-5 h-6 w-6 text-blue-400" />
          {searchQuery && (
            <button 
              type="button"
              onClick={clearSearch} 
              className="absolute right-28 top-5 text-gray-400 hover:text-gray-600 transition"
            >
              <X className="h-5 w-5" />
            </button>
          )}
          <button 
            ...
            className="absolute right-2 top-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition disabled:bg-blue-400 flex items-center gap-2 shadow-md"
          >
            {isSearching ? (
              <>
        ...
        <div className="mt-6">
          <button 
            ...
            className="inline-flex items-center px-4 py-2 bg-white border-2 border-blue-500 text-blue-500 rounded-full hover:bg-blue-50 transition font-medium shadow-sm"
          >
